...
export const createSupportTicket = async (req, res) => {
  try {
    ...
    const smtpUser = process.env.SMTP_USER || "divyamadhuratech@gmail.com";
    ...
    if (!smtpPass) {
      console.warn("WARNING: SMTP_PASS is not configured in backend/.env file.");
      return res.status(500).json({
        success: false,
        message: "SMTP_PASS is not configured in backend/.env. Please configure it to enable support emails."
      });
    }
